...
export default function AeronavesPage() {
  ...
  const handleDelete = async (id, matricula) => {
    if (!confirm(`¿Estás seguro de que deseas eliminar la aeronave ${matricula}?`)) {
      return;
    }
    ...
    if (error) {
      setErrorMsg(`Error al eliminar la aeronave: ${error.message}`);
    } else {
      setAeronaves((prev) => prev.filter((aero) => aero.id !== id));
    }
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-100 border-b">
                  ...
                  <th className="p-3 font-semibold text-gray-700">Acciones</th>
                </tr>
              </thead>
              <tbody>
                {aeronaves.map((aero) => (
                  <tr key={aero.id || aero.matricula} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-3">
                      <button
                        ...
                        disabled={deletingId === aero.id}
                        className="bg-red-500 hover:bg-red-600 text-white font-medium text-xs px-3 py-1 rounded transition duration-200 disabled:opacity-50"
                      >
                        {deletingId === aero.id ? 'Eliminando...' : 'Eliminar'}
                      </button>
                    </td>
                  </tr>
                ))}
